...
describe("reader development runtime contracts", () => {
  ...
  it("anchors desktop sticky chat and resets sticky geometry in the mobile stack", () => {
    ...
    expect(readerStyles).toMatch(/\.environment-dev \.book-chat\s*\{[^}]*top:\s*34px;[^}]*height:\s*calc\(100vh - 34px\);[^}]*max-height:\s*calc\(100vh - 34px\);/s);
